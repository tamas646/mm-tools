...
function restore_backup {
	...
	set +e
	exitcode="`compgen -G "${MM_DIR}/.backup/databases/*.sql" > /dev/null ; echo "$?"`"
	set -e
	if [[ "$exitcode" == "0" ]]
	then
		databases=(`ls "${MM_DIR}/.backup/databases/"*.sql`)
		j=0
		while [[ $j < ${#databases[@]} ]]
		do
